...
import { Clock, CheckCircle2, Star, ChevronRight, SearchX, User } from "lucide-react"
...
export function DentistaTratamentosLista() {
  const { screenParams, currentUser, tratamentos, pacientes, navigate } = useApp()
  ...
  const meusTratamentos = tratamentos.filter(
    (t) => t.dentistaId === dentista.id && t.status === filtro
  )
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {Object.entries(statusLabels).map(([key, label]) => {
            ...
            return (
              <button
                key={key}
                onClick={() => navigate("dentista-tratamentos", { filtro: key })}
        ...
        {meusTratamentos.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {meusTratamentos.map((tratamento) => {
              const paciente = pacientes.find((p) => p.id === tratamento.pacienteId)
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {paciente && (
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">{paciente.nome}</span>
                      </div>
                    )}
                  </div>
